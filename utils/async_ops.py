...
class OpNotStartedError(Exception):
    """In status-tracking APIs, it informs that the `AfterOp` has not
    started yet.
    """
    pass
...
class AsyncOp:
    ...
    * hashability
    ...
    def HasDone(self) -> bool:
        ...
        otherwise `False`. Raises `OpNotStartedError` if the operation
        has not started yet.
        """
        if self._future:
            return self._future.done()
        raise OpNotStartedError()
    
    def Cancel(self) -> None:
        """Cancels the operation. It does not have any effect if the
    ...
    def HasCanceled(self) -> bool:
        """Returns `True` if the operation has canceled, otherwise
        `False`. Raises `OpNotStartedError` if the operation has not
        started yet.
        """
        if self._future:
